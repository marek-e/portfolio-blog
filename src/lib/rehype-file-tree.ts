...
const fileTreeProcessor = rehype()
	...
	.use(function fileTree() {
		return (tree: Element, file) => {
			const directoryLabel = file.data.directoryLabel as string;
			...
			visit(tree, 'element', (node) => {
				...
				if (isDirectory) {
					icon.children.unshift(h('span', { class: 'sr-only' }, String(directoryLabel)));
				}
...
function getFileIcon(fileName: string) {
	...
	if (name in FileIcons) {
		return makeSVGIcon(FileIcons[name as keyof typeof FileIcons]);
	}
...
function getFileIconName(fileName: string) {
	const icon: string | undefined = definitions.files[fileName];
	if (icon) return icon;
	const extIcon = getFileIconTypeFromExtension(fileName);
