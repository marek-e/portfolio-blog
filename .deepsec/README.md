...
Currently configured project: `astro-perso-website` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/astro-perso-website/SETUP.md` to fill in
   `data/astro-perso-website/INFO.md`.
...
`--project-id astro-perso-website` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/astro-perso-website/`.
