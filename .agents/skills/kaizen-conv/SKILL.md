---
name: kaizen-conv
description: Use only when the user explicitly asks to analyze an agent conversation export for workflow friction
---
...
Use the conversation file path supplied with the skill invocation.
</path-to-conversation-file>
...
- User pasted CI `format:verify` failure output → the `validate` skill should have caught this
...
- **Read the relevant shared skills** in `.agents/skills/`
...
- Assistant tried wrong fixes twice before user provided correct command (`db:reset`)
  - Assistant didn't recognize that jOOQ type errors mean schema mismatch
    ...
    - The `validate` skill has no diagnostic guidance for common failures 2️⃣
...
- 2️⃣ `.agents/skills/validate/SKILL.md`: Add troubleshooting guidance: "If jOOQ compilation fails, run `npm run db:reset`"
- 3️⃣ `AGENTS.md`: Add rule: "After 2 failed fix attempts, stop and ask user for guidance"
